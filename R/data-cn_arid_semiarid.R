#' Table 2-2d: Runoff curve numbers for arid and semiarid rangelands
#'
#' A table containing curve numbers for arid and semiarid areas.
#'
#'
#'
#' @format A data.table data frame with 19 rows and 6 variables:
#' \describe{
#' \item{Cover type}{Type of agricultural ground cover}
#' \item{Hydrologic condition *2}{Hydrologic condition}
#' \item{Curve numbers for hydrologic soil group A *3}{Curve numbers for soil group A}
#' \item{Curve numbers for hydrologic soil group B}{Curve numbers for soil group B}
#' \item{Curve numbers for hydrologic soil group C}{Curve numbers for soil group C}
#' \item{Curve numbers for hydrologic soil group D}{Curve numbers for soil group D}
#' }
#'
#'
#' @source
#' United States Department of Agriculture Natural Resources Conservation Service Conservation Engineering Division, "Urban Hydrology for Small Watersheds Technical Release 55 (TR-55)", June 1986, \url{https://web.archive.org/web/20230810204711/https://directives.sc.egov.usda.gov/OpenNonWebContent.aspx?content=22162.wba} [Recovered with the Internet Archive: Wayback Machine]
#'
#'
#'
#'
"cn_arid_semiarid"
#> [1] "cn_arid_semiarid"
